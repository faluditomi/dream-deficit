...
The ChatClient window SHALL display exactly one user entry per chat user resolved from the current day's roster. Each entry SHALL show the user's profile picture, username, and the last message of the user's chat log, and SHALL own that user's chat log instance.

#### Scenario: Populating the user list

- **WHEN** a day whose roster contains two chat users is loaded
- **THEN** the ChatClient window displays exactly two user entries, each showing its user's profile picture, username, and last chat log message
